...
import { Flame, ChevronLeft, Pencil, Check, Shield, Target } from "lucide-react-native";
import {
  getPactById,
  getLogsForPact,
  getLogForDate,
...
function ActionBadge({ action, isDark }: { action: string; isDark: boolean }) {
  const style =
    action === "COMPLETE"
  ...
  const handleComplete = () => {
    ...
    if (!result.success) {
      setConfirmModal({
        visible: true,
        title: "Thông báo",
        message: result.message ?? "Không thể ghi nhận.",
  ...
  const handleEvolve = () => {
    if (!evolveGoalName.trim() || !evolveDeadline) {
      setConfirmModal({
        ...
        message: "Nhập đầy đủ Mục tiêu và chọn Hạn chót.",
        buttons: [{ text: t("common.ok") }],
      });
      return;
    }
    ...
    <SafeAreaView className={`flex-1 ${bg}`} edges={["bottom"]}>
      <ScrollView className="flex-1 px-4 pt-4">
        <View className="mb-4 flex-row items-center">
          ...
          <Text
            className={`flex-1 text-lg font-bold ${text}`}
            numberOfLines={1}
          >
            {pact.name}
          </Text>
          <Pressable
            onPress={() =>
              router.push({ pathname: "/pacts/edit", params: { id: pact.id } })
            }
          ...
          className={`mb-4 rounded-xl p-4 ${
            pact.goalName || pact.goalDeadline
              ? isDark
                ...
                : "border-2 border-amber-400/60 bg-amber-50"
              : card
          }`}
        >
          {pact.goalName || pact.goalDeadline ? (
            ...
            <Text className={`text-sm ${muted}`}>
              {getNoGoalPhraseByIndex(locale, noGoalQuoteIndex)}
            </Text>
          )}
          <View className="mt-3 flex-row items-center justify-between">
            <View className="flex-row items-center">
              <Flame color="#F97316" size={18} />
              <Text className="ml-1 font-bold text-orange-400">
                {pact.totalFire}
              </Text>
            </View>
          ...
          (pact.targetCount != null && pact.targetCount > 0) ? (
            <>
              <View className="mt-2 h-2 overflow-hidden rounded-full bg-slate-700">
                <AnimatedProgressBar progress={Math.min(progressPct, 100)} height={8} />
              </View>
        ...
        <Text className={`mb-2 font-medium ${muted}`}>
          {t("detail.recentLog")}
        </Text>
        {logs.length === 0 ? (
        ...
        >
          ...
          >
            ...
            <Pressable
              ...
              className="mb-2 rounded-xl bg-orange-500 py-3"
            >
              <Text className="text-center font-bold text-white">
                {t("detail.evolve")}
              </Text>
